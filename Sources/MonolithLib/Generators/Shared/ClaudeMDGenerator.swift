...
enum ClaudeMDGenerator {
    ...
    static func generateForPackage(config: PackageConfig) -> String {
        ...
        if config.hasDefaultIsolation {
            ...
            buildSection.append("")
            buildSection.append("```bash")
            buildSection.append("xcodebuild build -scheme \(config.name)-Package -destination '\(Defaults.simulatorDestination)' CODE_SIGNING_ALLOWED=NO")
            buildSection.append("xcodebuild test -scheme \(config.name)-Package -destination '\(Defaults.simulatorDestination)' CODE_SIGNING_ALLOWED=NO")
            buildSection.append("```")
            buildSection.append("")
