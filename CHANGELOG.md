...
### Changed
- **Generated projects now pin LumiKit 0.12.0** instead of 0.9.0, so a fresh scaffold starts on the current release rather than one that is merely new enough to compile. The requirement is emitted as `from:` / `upToNextMajorVersion`, so existing projects were already free to resolve forward; this only raises the declared floor. The hard compile floor is still 0.9.0, where `UIColor.lmk_dynamic(...)` landed.
- **The SnapKit default moved to 6.0.0 in lockstep with the LumiKit pin.** LumiKit 0.12.0 raises its own SnapKit requirement to `from: 6.0.0`, so a scaffold pinning SnapKit 5.x alongside it would fail dependency resolution; the two floors move together. The Lottie default moved to 4.6.1 as routine catch-up to the current release.
