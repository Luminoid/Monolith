...
### Fixed
- **`new package` can wire local-path dependencies at all**: `PackageSwiftGenerator` ignored `ExternalPackage.isLocalPath` and emitted `.package(url:)` unconditionally. Path-form entries carry no version requirement, so the output was `.package(url: "../LumiKit", )` — a manifest that fails to compile with *"missing argument for parameter 'from' in call"*, before any target is built. `SPMAppGenerator` had the correct `.package(name:path:)` branch since the flag shipped; it was never mirrored to the package generator. `generate(config:)` gains an optional `projectRoot:` (passed by `PackageProjectGenerator`) so absolute paths normalize to project-root-relative form, matching the app path. (Was critical: local SPM paths are how a package depends on a sibling framework in development, so `monolith new package` could not scaffold any multi-package workspace.)
- **Generated `Makefile` / `CLAUDE.md` / `README.md` referenced a scheme that does not exist**: `PackageConfig.xcodeBuildScheme` returned the bare package name whenever there were no executables and no test-helper targets. But Xcode only generates a `<Name>` scheme when some *target* is named `<Name>` — a package `Eluvium` with targets `EluviumCore` / `EluviumNet` / … yields `Eluvium-Package` plus one scheme per target and no `Eluvium`, so `make build` and `make test` failed on a fresh scaffold with *"does not contain a scheme named Eluvium"*. The umbrella is now also selected when no target carries the package name (the shape LumiKit, Prism, and Sophon all have), and the CLAUDE.md explainer names the real reason instead of claiming the package "mixes target kinds" and pointing readers at a named scheme that was never created. Five existing tests asserted the old behavior and were inverted.
- **Generated CLAUDE.md no longer claims `swift build` works on a UIKit-wired package**: the `defaultIsolation` branch closed with "Foundation-only targets can use `swift build` / `swift test`", but both invocations compile *every* target, so any package with a MainActor target (or a UIKit-based dep such as `LumiKitUI`) fails on a macOS host with `no such module 'UIKit'` — `swift test --filter` too, since the filter selects which tests run but still builds the whole package. The section now names the Foundation-only targets, shows the `swift build --target <Name>` form that actually works, and states plainly that the bare invocations do not. `ReadmeGenerator` was already correct (it switches to `xcodebuild` wholesale under `defaultIsolation`).
- **`--external-packages` path-form syntax was documented wrong**: README and `.claude/CLAUDE.md` showed `'LumiKit=path:../LumiKit'`. There is no `path:` keyword in `ExternalPackage.parse` — the whole token after `=` is the path, so that string set the dependency path to the literal `path:../LumiKit`. Corrected to `'LumiKit=../LumiKit'` (the form the doc comment and `AppConfigTests` already used), and the multi-product case is now documented: the must-be-consumed check matches product names, so declare one entry per consumed product with the `:packageName` tail and let `collectExternalDependencies` de-dupe them into a single `.package(...)` line.
...
- **Generated `#if targetEnvironment(macCatalyst)` blocks conform to `--ifdef indent`**: `MacCatalystGenerator`, `AppDelegateGenerator`, `SceneDelegateGenerator`, and `TabBarGenerator` emitted `#if` block contents at column 0, violating the generated `.swiftformat` (and the workspace convention shared by every shipped Lumi app), so a fresh Mac Catalyst scaffold failed its own `make check` under current SwiftFormat. Block contents are now indented one level, and `buildTabs()` no longer opens with a blank line. A new integration test runs `swiftformat --lint` against a generated app (previous tests only checked that `.swiftformat` existed).
- **Generated `cloudKitSharing` app no longer crashes on launch under `make test`**: the Core Data dual-store stack attached `cloudKitContainerOptions` to both stores unconditionally, forcing `NSPersistentCloudKitContainer` mirroring on at first launch. With no CloudKit entitlement applied (the generated `make test` builds with `CODE_SIGNING_ALLOWED=NO`, as does any unsigned or CI run), `NSCloudKitMirroringDelegate`'s async setup traps inside CloudKit and crashes the app host before the test bundle connects: the test target compiled but every run reported "Early unexpected exit ... Test crashed with signal trap before establishing connection." Sync is now opt-in behind a `cloudKitEnabledKey` UserDefaults flag that defaults off (a single local store, no mirroring), matching Petfolio's `PetCoreDataStack`; adopters flip it from a Settings toggle and `exit(0)` to reconfigure. Because the gated stack reads UserDefaults, the app's privacy manifest now declares `NSPrivacyAccessedAPICategoryUserDefaults` (CA92.1) when the Core Data dual-store path is generated; the SwiftData path, which never touches UserDefaults, stays empty. (Was critical, and slipped past the prior fix round: only running the generated tests catches a launch crash, since substring and `swiftformat --lint` checks pass against output that crashes at runtime.)
