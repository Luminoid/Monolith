...
- **`--category` flag on `new app`** — App Store category (`public.app-category.productivity`, etc.). Required for Mac App Store distribution. Defaults to `public.app-category.utilities`. Written into BOTH the Info.plist as `LSApplicationCategoryType` and the XcodeGen build settings as `INFOPLIST_KEY_LSApplicationCategoryType`. Both are required: the plist makes the file self-describing, the build-setting form is what `xcodebuild archive` consults — missing the build setting produces a non-fatal `warning: No App Category is set for target` that App Store Connect rejects on upload (workspace lesson; Petfolio regressed twice).
...
- **`LSApplicationCategoryType` lives in BOTH the Info.plist AND the XcodeGen build settings** — the previous "Info.plist only" approach is fine at runtime, but `xcodebuild archive` for Mac Catalyst (and even iOS-only targets) emits `warning: No App Category is set for target` when the `INFOPLIST_KEY_LSApplicationCategoryType` build setting is missing. The warning isn't an `error:`, so `make release-mac` exits 0, and then App Store Connect rejects the upload. The build-setting form *does* land in the final pbxproj via XcodeGen — the historical "won't merge with `GENERATE_INFOPLIST_FILE: NO`" hypothesis was wrong (Pharos regenerated and verified). Petfolio regressed twice on this; the rule is "both forms always."
...
- **`AppDelegate.modelContainer` is now non-optional (`ModelContainer!`) and `createModelContainer()` `fatalError`s on init failure** — matches Apple's sample-code pattern and the workspace lesson "container failures must be fatal in init." The pre-change `ModelContainer?` + logged-and-ignored failure pattern produced misleading "no persistent stores" crashes miles from the cause. When `lumiKit` is also enabled the failure path uses `LMKLogger.error(..., category: LMKLogger.LogCategory.data)`; otherwise plain `print()`. `import LumiKitCore` is now emitted alongside `import LumiKitUI` (LumiKitUI depends on LumiKitCore but doesn't re-export it).
- **`SceneDelegate` SwiftData guard collapses to the tabs path only** — the no-tabs SwiftData configuration no longer emits a guard or `import SwiftData`. With AppDelegate's container non-optional + `fatalError`, the historical `guard ... modelContainer != nil` defensive check was dead code (the scene only reaches `willConnectTo` after `didFinishLaunching` succeeded, which now means the container exists). The tabs path still binds `let modelContainer = ...` for `MainTabBarController(modelContainer:)`.
...
- **`DataPublisher.swift` sample singleton from the `combine` feature** — every adopter deleted it on first commit (no real consumers, no integration test coverage, just a `static let shared` PassthroughSubject demo). `CombineGenerator.generateDataPublisher()` is gone; `generateAsyncService()` (the actual reference template for Task cancellation) stays. The `combine` feature now writes one file (`AsyncService.swift`), not two.
